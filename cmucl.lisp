;;;; cmucl.lisp
#-cmucl (error "This source file is only meant for CMU Common Lisp.")
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (shadowing-import
   '(ext:function-information ext:variable-information
     ext:declaration-information)
   '#:introspect-environment)
  (export '(ext:function-information ext:variable-information
	    ext:declaration-information )
	  '#:introspect-environment))
...
(defun parse-macro (name lambda-list body &optional env)
  (let* ((whole-var (gensym "WHOLE")))
    (lisp::parse-defmacro lambda-list whole-var body name 'defmacro :environment env)))
...
(defun constant-form-value (form &optional env)
  (let ((expanded-form (macroexpand form env)))
	...
	(eval:internal-eval expanded-form t env)
